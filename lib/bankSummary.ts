...
/** Basic cell/row/table types to avoid `any` */
...
function findHeaderRow(aoa: AOA): number {
...
function indexCols(header: Row): Cols {
...
function coerceDate(v: Cell): Date | null {
...
function toNumber(x: Cell): number {
  const n = Number(String(x ?? "").toString().replace(/,/g, ""));
  return Number.isFinite(n) ? n : 0;
}

/** Excludes rows where Remarks == "Interbank" (case-insensitive) */
function accumulate(
  rows: AOA,
  startRow: number,
  cols: Cols,
  ...
  for (let r = startRow; r < rows.length; r++) {
    ...
    // amount
    const cr = cols.cr != null ? toNumber(row[cols.cr]) : 0;
    const dr = cols.dr != null ? toNumber(row[cols.dr]) : 0;

    const amt = kind === "CR" ? cr : dr;
    if (!amt) continue;
...
function buildTable(title: string, acc: Acc): (string | number)[][] {
  const header: (string | number)[] = [title, ...acc.months, "Total", "% Contributions"];
...
export function buildSummarySheet(ws: XLSX.WorkSheet): XLSX.WorkSheet {
  const aoa = XLSX.utils.sheet_to_json({ ...ws }, { header: 1, raw: true }) as AOA;
